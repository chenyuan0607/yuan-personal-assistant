import { budgetStatus, buildDailyLedgerSummary, createBackup, filterRecords, formatMoney, parseBackup, rankCategories, sortRecords, summarizeMonth, validateRecord } from "./ledger.js";
...
export async function initLedger({ onSummary = async () => {}, getDeviceName = () => "我的手机" } = {}) {
  const emitSummary = async (date = localDate()) => onSummary(buildDailyLedgerSummary(records, settings, date, getDeviceName()));
  document.querySelector("#month-filter").value = localDate().slice(0, 7);
  [records, customCategories, templates] = await Promise.all([listStore("records"), listStore("categories"), listStore("templates")]); settings = Object.fromEntries((await listStore("settings")).map((item) => [item.id, item.value])); updateCategories("expense"); render(); await emitSummary();
  ...
  document.querySelector("#delete-record").addEventListener("click", async () => { const id = document.querySelector("#record-id").value; const removed = records.find((item) => item.id === id); if (!id || !removed || !confirm("确定删除这笔账目吗？")) return; await deleteStore("records", id); records = records.filter((item) => item.id !== id); document.querySelector("#record-dialog").close(); render(); await emitSummary(removed.date); toast("账目已删除"); });
  document.querySelector("#record-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; const id = document.querySelector("#record-id").value; const old = records.find((item) => item.id === id); const record = validateRecord({ id: id || crypto.randomUUID(), type: form.elements.type.value, amountCents: Math.round(Number(document.querySelector("#amount").value) * 100), category: document.querySelector("#category").value, date: document.querySelector("#record-date").value, note: document.querySelector("#note").value, createdAt: old?.createdAt || new Date().toISOString() }); await putStore("records", record); records = records.filter((item) => item.id !== record.id).concat(record); if (document.querySelector("#save-template").checked) { const template = { id: crypto.randomUUID(), title: record.note || record.category, type: record.type, amountCents: record.amountCents, category: record.category }; templates.push(template); await putStore("templates", template); } document.querySelector("#record-dialog").close(); render(); if (old?.date && old.date !== record.date) await emitSummary(old.date); await emitSummary(record.date); toast("账目已保存"); });
